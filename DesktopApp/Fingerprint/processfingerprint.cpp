...
void ProcessFingerprint::match(cv::Mat &leftImage ,cv::Mat &rightImage)
{
    std::vector<Minutia> leftMinutiae;
    cv::Mat leftTemp = leftImage.clone();
    obtainMinutiaeFromSourceImage(leftTemp,304, leftMinutiae, 1, NotKnown);

    std::vector<Minutia> rightMinutiae;
    cv::Mat rightTemp = rightImage.clone();
    obtainMinutiaeFromSourceImage(rightTemp,304, rightMinutiae, 2, NotKnown);

    /*
    cv::Mat comparator = cv::Mat(304, 304,CV_8UC3);
    comparator(cv::Rect(0,0,304,304)) = cv::Scalar::all(255);

    MinutiaExtractor mextractor;
    mextractor.compareFingerprintMinutiae(leftMinutiae,comparator,1);
    mextractor.compareFingerprintMinutiae(rightMinutiae,comparator,2);
    mextractor.showMinutiae(leftMinutiae,leftTemp);
    cv::imshow("Imangen: Comparacion de huellas dactilares",comparator);
    */
    Matching matching;
    float matchingValue = matching.match(leftMinutiae,rightMinutiae);
    qDebug() << matchingValue;

    emit progressValueCompareFingerprints(QString::number(matchingValue));
    emit finishedFingerprintComparison();
}
